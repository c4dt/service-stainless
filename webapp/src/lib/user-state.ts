...
export class SourceFile extends UserEvmInfo {
  ...
  constructor(readonly name: string, readonly contents: string) {
    super();
  }
...
export class UIEvmAccount extends UserEvmInfo {
  ...
  constructor(readonly wrapped: EvmAccount) {
    super();
  }
...
export class Project extends UserEvmInfo {
  static deserialize(obj: any): Project {
    const sourceFiles = SelectableColl.deserializeColl<SourceFile>(
      obj.sourceFiles,
      SourceFile
    );
    const contracts = SelectableColl.deserializeColl<UIEvmContract>(
      obj.contracts,
      UIEvmContract
    );
  ...
  constructor(
    ...
    sourceFiles: SelectableColl<SourceFile>,
...
export class UserState extends UserEvmInfo {
  ...
  static deserialize(obj: any): UserState {
    ...
    if (version < this.currentVersion) {
      Log.lvl2(
        `UserState version not up to date: ${version} vs ${this.currentVersion}`
      );
    ...
    const accounts = SelectableColl.deserializeColl<UIEvmAccount>(
      obj.accounts,
      UIEvmAccount
    );
    const projects = SelectableColl.deserializeColl<Project>(
      obj.projects,
      Project
    );
  ...
  updateProjects(projectData: any[]) {
    for (const pData of projectData) {
      ...
      const sourceFiles = pData.files.map(
        (file: any) => new SourceFile(file.name, file.contents)
      );
  ...
  set contracts(contracts: UIEvmContract[]) {
    this._projects.selected.contracts = new SelectableColl<UIEvmContract>(
      contracts
    );
    this.save();
  }
  ...
  get instanceSelectedIndex(): number {
    if (
      this._projects.selected === undefined ||
      this._projects.selected.contracts.selected === undefined
    ) {
      return undefined;
    }
  ...
  get transactionSelectedIndex(): number {
    ...
    return this._projects.selected.contracts.selected.transactions
      .selectedIndex;
  }
