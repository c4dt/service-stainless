...
export class WebSocketConnection implements IConnection {
    ...
    this.ws.onmessage = (msg: MessageEvent) => {
      ...
      this.received.add(
        new Promise((resolve, reject) => {
          Log.lvl4(
            `WebSocketConnection.ws.onmessage: resolving with ${msg.data}`
          );
  ...
  async sendmsg(msg: Uint8Array): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      Log.lvl4(
        `WebSocketConnection.sendmsg: sending ${msg} with ws in state ${this.ws.readyState}`
      );
      this.ws.send(msg);
      resolve();
    });
  ...
  async recvmsg(): Promise<Uint8Array> {
    ...
    Log.lvl4(
      `WebSocketConnection.recvmsg: getting from pool of size ${this.received.size}`
    );
