...
export class StainlessRPC {
  ...
  constructor(srvid: ServerIdentity) {
    this.serviceAddress =
      srvid.getWebSocketAddress() + "/" + StainlessRPC.serviceName;
  }

  async verify(sourceFiles: {
    [_: string]: string;
  }): Promise<proto.VerificationResponse> {
    const conn = new WebSocketConnection(
      this.serviceAddress + "/VerificationRequest"
    );
  ...
  async genBytecode(sourceFiles: {
    [_: string]: string;
  }): Promise<proto.BytecodeGenResponse> {
    const conn = new WebSocketConnection(
      this.serviceAddress + "/BytecodeGenRequest"
    );
